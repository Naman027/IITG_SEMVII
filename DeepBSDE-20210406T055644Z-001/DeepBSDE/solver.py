...
class BSDESolver(object):
    ...
    def train(self):
        ...
        _, full_soln = self.model(valid_data, training=False, full = True)

        return np.array(training_history), full_soln

    def loss_fn(self, inputs, training):
        dw, x = inputs
        y_terminal, _ = self.model(inputs, training)
...
class NonsharedModel(tf.keras.Model):
    ...
    def call(self, inputs, training, full = False):
        dw, x = inputs
        ...
        for t in range(0, self.bsde.num_time_interval-1):
            ...
            if full:
                ...
                # Assuming that x[i, :, t] is a vector, and y[i] is a real number

                for i in range(N):
                    full_soln.append((time_stamp[t], list(x[i, :, t].numpy()), y[i][0].numpy()))
