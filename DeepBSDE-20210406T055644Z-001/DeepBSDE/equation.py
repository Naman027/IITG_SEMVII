...
class HJBLQ(Equation):
    ...
    def sample(self, num_sample):
        ...
        x_sample[:, :, 0] = np.random.randn(num_sample, self.dim)
...
class BlackScholesCall(Equation):

    def __init__(self, eqn_config):
        super(BlackScholesCall, self).__init__(eqn_config)
        #self.x_init = np.zeros(self.dim)
        self.x_init = eqn_config.x_init
        self.sigma = eqn_config.sigma
        self.r = eqn_config.r
        self.K = eqn_config.K
        self.x_init_range = eqn_config.x_init_range
    ...
    def sample(self, num_sample):
        ...
        for i in range(self.num_time_interval):
            x_sample[:, :, i + 1] = x_sample[:, :, i] + \
                ...
                self.r * self.delta_t * x_sample[:, :, i]
    ...
    def g_tf(self, t, x):
        return tf.math.maximum(x - self.K, 0)


class AsianOption(Equation):

    def __init__(self, eqn_config):
        super(AsianOption, self).__init__(eqn_config)
        #self.x_init = np.zeros(self.dim)
        self.x_init = eqn_config.x_init
        self.sigma = eqn_config.sigma
        self.r = eqn_config.r
...
# x = [S A]^T = [S_0, 0]^T
    ...
    def sample(self, num_sample):
        ...
        x_sample[:, 0, 0] = np.ones([num_sample]) * self.x_init
        x_sample[:, 1, 0] = np.zeros(num_sample)

# mu(t, x) = [rx_1 x_1]^T
# sigma(t, x) = [self.sigma * x_1, 0; 0, 0]

        for i in range(self.num_time_interval):
            x_sample[:, 0, i + 1] = x_sample[:, 0, i] + \
                self.sigma * x_sample[:, 0, i] * dw_sample[:, 0, i] + \
                self.r * self.delta_t * x_sample[:, 0, i]
            x_sample[:, 1, i + 1] = x_sample[:, 1, i] + self.delta_t * x_sample[:, 0, i]
    ...
    def g_tf(self, t, x):
        return tf.math.maximum(x[1] - self.K, 0)
