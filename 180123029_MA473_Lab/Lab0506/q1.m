...
function Q1
	...
	U = Crank_Nicolson(@fun, @f, @g1, @g2, T, K, r, sig, delta, q, qd, x_min, x_max, h, k, m, n, X, Tau);
	figure; plot(S(indices), U(end, indices)); hold on; ttemp = transform(g(X, Time(end), q, qd), X, Time(end), q, qd, K); plot(S(indices), ttemp(indices)); hold off;
...
function [U] = Crank_Nicolson(fun, f, g1, g2, T, K, r, sig, delta, q, qd, x_min, x_max, h, k, m, n, X, Tau)
